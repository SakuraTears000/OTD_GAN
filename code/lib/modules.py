...
############   GAN   ############
def train(dataloader, netG, netD, netC, text_encoder, text_encoder_obj, image_encoder, obj_encoder, optimizerG, optimizerD, scaler_G, scaler_D, PL, args):
    ...
    for step, data in enumerate(dataloader, 0):
        ...
        with torch.cuda.amp.autocast() if args.mixed_precision else dummy_context_mgr() as mpc:
            # prepare_data
            real, obj_f, obj_tk, captions, CLIP_tokens, sent_emb, words_embs, sent_emb_with_obj, words_embs_with_obj, keys = prepare_data(data, text_encoder, text_encoder_obj, obj_encoder, PL,  device)
            real = real.requires_grad_()
            # obj_f = obj_f.requires_grad_()
            # obj_tk = obj_tk.requires_grad_()  # <<<<<<
            # sent_emb = sent_emb.requires_grad_()
            # words_embs = words_embs.requires_grad_()
            sent_emb = sent_emb_with_obj.requires_grad_()
            # predict real
...
def MA_GP_MP(img, sent, out, scaler):
    ...
    with torch.cuda.amp.autocast():
        ...
        d_loss_gp =  4.0 * torch.mean((grad_l2norm) ** 6)
    return d_loss_gp
...
def sample(dataloader, netG, text_encoder, text_encoder_with_obj, obj_encoder, PL, save_dir, device, multi_gpus, z_dim, cond_dim, times):
    netG.eval()
    for time in range(times):
        for step, data in enumerate(dataloader, 0):
            ...
            # (1) Prepare_data
            ######################################################
            real, obj_f, obj_tk, captions, CLIP_tokens, sent_emb, words_embs, sent_emb_with_obj, words_embs_with_obj, keys = prepare_data(data, text_encoder, text_encoder_with_obj, obj_encoder, PL, device)
            ######################################################
            ...
            with torch.no_grad():
                noise = torch.randn(batch_size, z_dim).to(device)
                fake_imgs = netG(noise, sent_emb_with_obj, eval=True).float()  # <<<<<<<<
                fake_imgs = torch.clamp(fake_imgs, -1., 1.)
                if multi_gpus==True:
                    batch_img_name = 'step_%04d.png'%(step)
                    batch_img_save_dir  = osp.join(save_dir, 'batch', str('gpu%d'%(get_rank())), 'imgs', str('time%d'%(time)))
                    batch_img_save_name = osp.join(batch_img_save_dir, batch_img_name)
                    batch_txt_name = 'step_%04d.txt'%(step)
                    batch_txt_save_dir  = osp.join(save_dir, 'batch', str('gpu%d'%(get_rank())), 'txts', str('time%d'%(time)))
                    batch_txt_save_name = osp.join(batch_txt_save_dir, batch_txt_name)
                else:
                    batch_img_name = 'step_%04d.png'%(step)
                    batch_img_save_dir  = osp.join(save_dir, 'batch', 'imgs', str('time%d'%(time)))
                    batch_img_save_name = osp.join(batch_img_save_dir, batch_img_name)
                    batch_txt_name = 'step_%04d.txt'%(step)
                    batch_txt_save_dir  = osp.join(save_dir, 'batch', 'txts', str('time%d'%(time)))
                    batch_txt_save_name = osp.join(batch_txt_save_dir, batch_txt_name)
                ...
                for j in range(batch_size):
                    ...
                    ######################################################
                    if multi_gpus==True:
                        single_img_name = 'gpu_%d_time_%d_step_%04d_batch_%04d.png'%(get_rank(), time, step, j)
                        single_img_save_dir = osp.join(save_dir, 'single')
                        single_img_save_name = osp.join(single_img_save_dir, single_img_name)
                    else:
                        single_img_name = 'time_%d_step_%04d_batch_%04d.png'%(time, step, j)
                        single_img_save_dir  = osp.join(save_dir, 'single')
                        single_img_save_name = osp.join(single_img_save_dir, single_img_name)
...
def sample_with_fixed_noise(dataloader, netG, text_encoder, text_encoder_with_obj, obj_encoder, PL, save_dir, device, multi_gpus, z_dim, cond_dim, times):
    netG.eval()
    for time in range(times):
        fixed_noise = torch.randn((times, z_dim))
        for step, data in enumerate(dataloader, 0):
            ######################################################
            # (1) Prepare_data
            ######################################################
            ...
            batch_size = sent_emb.size(0)
            with torch.no_grad():
                noise = fixed_noise[time].repeat(batch_size, 1).to(device)
                fake_imgs = netG(noise, sent_emb_with_obj, eval=True).float()  # <<<<<<<<
                fake_imgs = torch.clamp(fake_imgs, -1., 1.)
                if multi_gpus==True:
                    batch_img_name = 'step_%04d.png'%(step)
                ...
                for j in range(batch_size):
                    ...
                    ######################################################
                    if multi_gpus==True:
                        single_img_name = '%s_gpu_%d_time_%d_step_%04d_batch_%04d.png'%(src_name, get_rank(), time, step, j)
                        single_img_save_dir = osp.join(save_dir, 'single')
                        single_img_save_name = osp.join(single_img_save_dir, single_img_name)
                    else:
                        single_img_name = '%s_time_%d_step_%04d_batch_%04d.png'%(src_name,time, step, j)
                        single_img_save_dir  = osp.join(save_dir, 'single')
                        single_img_save_name = osp.join(single_img_save_dir, single_img_name)
                    mkdir_p(single_img_save_dir)
                    im.save(single_img_save_name)
...
def sample_one_batch(noise, noise_o, sent, sent_with_obj, obj_tk, obj_f, netG, multi_gpus, epoch, img_save_dir, writer):
    if (multi_gpus==True) and (get_rank() != 0):
        None
    else:
        netG.eval()
        with torch.no_grad():
            B = noise.size(0)
            fixed_results_train = generate_samples(noise[:B//2], noise_o[:B//2],  sent_with_obj[:B//2], obj_tk[:B//2], obj_f[:B//2], netG).cpu()
            torch.cuda.empty_cache()
            fixed_results_test = generate_samples(noise[B//2:], noise_o[B//2:], sent_with_obj[B//2:],  obj_tk[B//2:], obj_f[B//2:], netG).cpu()
            torch.cuda.empty_cache()
...
def generate_samples(noise, noise_o, caption, otk, of, model):
    with torch.no_grad():
