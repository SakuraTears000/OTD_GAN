...
def cal_mask_prob(cur_epoch, max_epoch):
    max_prob = 0.3
    min_prob = -0.3
    k = (max_prob-min_prob)/0.6*max_epoch
    return min(max_prob, k * cur_epoch + min_prob)

def choose_model(model):
    '''choose models
    '''
    model = importlib.import_module(".%s"%(model), "models")
    NetG, NetD, NetC, CLIP_IMG_ENCODER, CLIP_TXT_ENCODER, CLIP_TXT_ENCODER_WITH_OBJ, CLIP_OBJ_ENCODER, PL= \
        model.NetG, model.NetD, model.NetC, model.CLIP_IMG_ENCODER, model.CLIP_TXT_ENCODER, \
        model.CLIP_TXT_ENCODER_WITH_OBJ, model.CLIP_OBJ_ENCODER, model.PromptLearner
    return NetG,NetD,NetC,CLIP_IMG_ENCODER, CLIP_TXT_ENCODER, CLIP_TXT_ENCODER_WITH_OBJ, CLIP_OBJ_ENCODER, PL
...
def load_prompt_learner(pl, path, multi_gpus, train=False):
