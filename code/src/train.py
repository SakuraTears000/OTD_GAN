...
from lib.utils import mkdir_p,get_rank,merge_args_yaml,get_time_stamp,save_args,cal_mask_prob
from lib.utils import load_models_opt,save_models_opt,save_models,load_npz,params_count,load_prompt_learner
from lib.perpare import prepare_dataloaders,prepare_models
from lib.modules import sample_one_batch as sample, test as test, train as train, sample as test_sample
...
def parse_args():
    ...
    parser.add_argument('--cfg', dest='cfg_file', type=str, default='../cfg/rsicd.yml',
...
def main(args):
    ...
    CLIP4trn, CLIP4evl, image_encoder, text_encoder, text_encoder_obj, obj_encoder, netG, netD, netC, PL = prepare_models(args)
    print('**************G_paras: ',params_count(netG))
    print('**************D_paras: ',params_count(netD)+params_count(netC))
    PL = load_prompt_learner(PL, args.pl_path, args.multi_gpus)
    fixed_img, fixed_obj_f, fixed_obj_tk, fixed_sent, fixed_sent_with_obj, fixed_z, fixed_z_o= get_fix_data(train_dl, valid_dl, text_encoder, text_encoder_obj, obj_encoder, PL, args)
    if (args.multi_gpus==True) and (get_rank() != 0):
        None
    else:
    ...
    # Start training
    gen_interval,save_interval,test_interval = args.gen_interval,args.save_interval,args.test_interval
    #torch.cuda.empty_cache()
    # start_epoch = 1
    epoch_thresh = 499
    save_path = osp.join(ROOT_PATH, 'test_sample/1227'
                                    '/k_4')  # <--------------------------------------
    ...
    for epoch in range(start_epoch, args.max_epoch, 1):
        ...
        torch.cuda.empty_cache()
        train(train_dl, netG, netD, netC, text_encoder, text_encoder_obj, image_encoder, obj_encoder, optimizerG, optimizerD, scaler_G, scaler_D, PL, args)
        torch.cuda.empty_cache()
        # save
        if epoch > epoch_thresh and epoch%save_interval==0:
            save_models_opt(netG, netD, netC, optimizerG, optimizerD, epoch, args.multi_gpus, args.model_save_file)
            torch.cuda.empty_cache()
        # sample
        if epoch%gen_interval==0:
            sample(fixed_z, fixed_z_o, fixed_sent, fixed_sent_with_obj, fixed_obj_tk, fixed_obj_f, netG, args.multi_gpus, epoch, args.img_save_dir, writer)
            torch.cuda.empty_cache()
        # test
        if epoch > epoch_thresh and epoch%test_interval==0:
            # torch.cuda.empty_cache()
            test_save_dir = osp.join(save_path, 'epoch_{}'.format(epoch))
            test_sample(valid_dl, netG, text_encoder, text_encoder_obj, obj_encoder,  PL, test_save_dir, args.device,
                        args.multi_gpus, args.z_dim, args.cond_dim, args.sample_times)
            torch.cuda.empty_cache()
        if (args.multi_gpus==True) and (get_rank() != 0):
            None
        else:
