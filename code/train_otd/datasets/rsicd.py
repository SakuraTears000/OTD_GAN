...
class RSICD(DatasetBase):
    dataset_dir = "objects_for_clip"
    cname2lab = {'baseball_field': 0,'boat':1,'bridge':2,'car':3,'playground':4,'storage_tank':5}
    ...
    def read_data(self, split):
        ...
        items = []
        cls = os.listdir(filepath)
        for c in cls:
            path = os.path.join(filepath, c)
            for imname in os.listdir(path):
                label = self.cname2lab[c]
                impath = os.path.join(path, imname)
                item = Datum(impath=str(impath), label=int(label), classname=str(c))
                items.append(item)
